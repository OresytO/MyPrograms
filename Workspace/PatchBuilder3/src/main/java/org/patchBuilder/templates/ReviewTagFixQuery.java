...
public class ReviewTagFixQuery extends AbstractPatchQuery
{
  ...
  public ReviewTagFixQuery build(Map<String, List<String>> tagMap, List<String> localeList)
  {
    ...
    // CreateTable.begin(tableName,
    // "AS SELECT ClientID, DimensionExternalID, Label FROM Tag Where ClientID = @CID");
    // query.append(CreateTable.end());
    //
    // tableName = "Tag";
    ...
    for (String tag : tagMap.keySet())
    {
      ...
      for (int i = 0; i < list.size(); i++)
        query.append(CONST.TAB2).append("WHEN ").append(CONST.QUO).append(list.get(i)).append(CONST.QUO).append(" THEN '").append(localeList.get(i)).append("'").append(CONST.EOL);
