...
void ComboDropCur(TComboBox * ComObj, TADODataSet * DataSet, int * &Courier_id)
{
    ...
    DataSet->CommandText = "SELECT name_storage_number FROM `courier`;";
    DataSet->Open();
...
{
    String dataStr = "INSERT INTO `delivery` (";
...
{
	String SQLstr = "SELECT cities_id, `name` FROM `cities`;";
...
{
	String SQLstr = "SELECT contragent_id, `name` FROM `contragent`;";
...
{
	String SQLstr = "SELECT status_id, `status` FROM `status`;";
