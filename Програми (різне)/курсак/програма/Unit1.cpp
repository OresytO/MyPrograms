...
int main()
{
     ...
     xk = 2.38 * log( Nv + 0.5); 	cout<<setw(14) << "Значення xk = " <<setw(6) <<xk<< "; ";
     dx = (xk - xp)/8; 	cout<<setw(14) << "Значення dx = " <<setw(6) <<dx<< "." <<endl;
     yp = -0.74 * log( Nv + 1.3); 	cout<< "Значення yp = " <<setw(6) <<yp<< "; ";
     yk = 0.86 * log( Nv + 1.7); 	cout<<setw(14) << "Значення yk = " <<setw(6) <<yk<< "; ";
     dy = (yk - yp)/10; 	cout<<setw(14) << "Значення dy = " <<setw(6) <<dy<< "." <<endl;
     z = -1.3 * log( Nv + 1.5) ; 	cout<< "Значення z = " <<setw(7) << z << "." <<endl<<endl;
...
double Fn_ai(double x, int i)
{
     double c1 = pow(cos(i + 5 * x), 2) / (i + x * exp(i + 3));
     double c2 = log(pow(i, 1.4) + 0.8);
     return c1 + c2;
}
