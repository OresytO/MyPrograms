...
void __fastcall TFAddCourier::FormCreate(TObject *Sender)
{
	...
	ComboBox1->Items->Add("посвідчення водія");
...
{
	String SQLstr = "SELECT payment_id, payment FROM `rebok2_db1`.`payment`";
...
{
	//exit(1);
	String dataStr = "INSERT INTO `rebok2_db1`.`courier` (";
    String colStr, valueStr = " '";

	if(Edit1->Text.IsEmpty()){
    	ShowMessage("Введіть назву компанії");
    	return;
    } else {
    ...
    if(Edit3->Text.IsEmpty()){
    	ShowMessage("Введіть адресу складу");
        return;
    } else {
    	colStr += ", adress";
        valueStr += ", '" + Edit3->Text + '\'';
    ...
    if(Edit4->Text.IsEmpty()){
    	ShowMessage("Введіть № телефону складу");
        return;
    } else {
    	colStr += ", phone_1";
        valueStr += ", '" + Edit4->Text + '\'';
    ...
    if(ComboBox1->Text.IsEmpty()){
    	ShowMessage("Вкажіть що є для даного перевізника посвідченням особи");
        return;
    } else {
    ...
    if(ComboBox2->Text.IsEmpty()){
    	ShowMessage("Виберіть вид розрахунку");
        return;
    } else {
        colStr += ", payment_id";
